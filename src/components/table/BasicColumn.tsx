...
import type { MealCateogies, Nutrients } from "@/app/types";
import type { Servings } from "@/app/types";
...
const roundDown = (num: number | undefined, decimals = 0): number => {
	if (typeof num !== "number" || num === undefined) return Number.NaN;
...
const createRoundedCell = <T extends keyof Food["nutrients"]>(
	...
	return ({ row }) => {
		...
		return Number.isNaN(roundedValue) ? "-" : roundedValue.toFixed(decimals);
	};
};
