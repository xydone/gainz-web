...
export function lerp(data: (number | null)[]): number[] {
	...
	for (let i = 0; i < n; i++) {
		if (result[i] !== null) {
			...
			if (end < n) {
				const x0 = start;
				const y0 = result[start] as number;
				const x1 = end;
				const y1 = result[end] as number;
...
export function imperialToMetric(
	...
	return {
		meters: Number.parseFloat(meters.toFixed(2)), // Round to 2 decimal places
		centimeters: Number.parseFloat(centimeters.toFixed(2)),
	};
}
...
export function centimetersToImperial(centimeters: number): {
	...
	const inches = Number.parseFloat((totalInches % 12).toFixed(2));
